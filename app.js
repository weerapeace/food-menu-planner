...
function bindRecipeDialogs() {
  ...
  document.getElementById('manageRecipeCategoriesBtn').addEventListener('click', openRecipeCategoryDialog);
  ...
  document.getElementById('closeRecipeCategoryDialog').addEventListener('click', closeRecipeCategoryDialog);
  document.getElementById('saveRecipeCategoryBtn').addEventListener('click', saveRecipeCategoryFromDialog);
...
async function loadData() {
  ...
  state.customRecipeCategories = loadCustomRecipeCategories();
...
function loadCustomRecipeCategories() {
  try {
    return JSON.parse(localStorage.getItem(RECIPE_CATEGORY_KEY) || '[]');
  } catch (error) {
    return [];
  }
}
...
function persistLocalMirrors() {
  ...
  localStorage.setItem(RECIPE_CATEGORY_KEY, JSON.stringify(uniqueSorted(state.customRecipeCategories)));
}
...
function getAllRecipeCategories() {
  return uniqueSorted(
    state.data.recipes.map(function(item) { return item.category; })
      .concat(state.customRecipeCategories || [])
  );
}
...
function renderFloatingCart() {
  ...
  button.classList.toggle('is-hidden', state.activeTab === 'cart' || state.activeTab === 'home');
}
...
function openRecipeCategoryDialog() {
  document.getElementById('recipeCategoryManagerInput').value = '';
  renderRecipeCategoryManager();
  document.getElementById('recipeCategoryDialog').showModal();
}

function closeRecipeCategoryDialog() {
  document.getElementById('recipeCategoryDialog').close();
}

function renderRecipeCategoryManager() {
  const container = document.getElementById('recipeCategoryManagerList');
  const categories = getAllRecipeCategories();
  if (!categories.length) {
    container.innerHTML = '<div class="empty-state">ยังไม่มีประเภทอาหาร</div>';
    return;
  }

  container.innerHTML = categories.map(function(category) {
    const usageCount = state.data.recipes.filter(function(recipe) {
      return recipe.category === category;
    }).length;
    return [
      '<div class="category-row">',
      '<div><strong>' + escapeHtml(category) + '</strong><div class="detail-note">ใช้อยู่ใน ' + usageCount + ' เมนู</div></div>',
      '<div class="actions compact-actions">',
      '<button class="secondary" type="button" onclick="editRecipeCategoryName(' + JSON.stringify(category).replace(/"/g, '&quot;') + ')">แก้ไข</button>',
      '<button class="ghost" type="button" onclick="deleteRecipeCategoryName(' + JSON.stringify(category).replace(/"/g, '&quot;') + ')">ลบ</button>',
      '</div>',
      '</div>',
    ].join('');
  }).join('');
...
function saveRecipeCategoryFromDialog() {
  const input = document.getElementById('recipeCategoryManagerInput');
  ...
  if (!state.customRecipeCategories.includes(name) && !state.data.recipes.some(function(recipe) { return recipe.category === name; })) {
    state.customRecipeCategories.push(name);
    persistLocalMirrors();
    populateRecipeCategoryOptions();
    renderRecipeCategoryManager();
  }
...
async function editRecipeCategoryName(oldName) {
  const nextName = window.prompt('แก้ไขชื่อประเภทอาหาร', oldName);
  if (!nextName) return;
  ...
  raw.recipes = raw.recipes.map(function(recipe) {
    if (recipe.category === oldName) {
      const updated = Object.assign({}, recipe, { category: trimmed });
      affectedRecipes.push(updated);
      return updated;
    }
    return recipe;
  });

  state.customRecipeCategories = uniqueSorted(
    state.customRecipeCategories.map(function(category) {
      return category === oldName ? trimmed : category;
    }).concat(trimmed)
  );
  ...
  applyLoadedState({
    ...
    source: 'local',
    statusText: 'อัปเดตประเภทอาหารแล้ว',
  });
...
async function deleteRecipeCategoryName(categoryName) {
  if (!window.confirm('ลบประเภทอาหารนี้ใช่ไหม เมนูที่อยู่หมวดนี้จะถูกย้ายเป็นไม่ระบุประเภท')) {
    return;
  }
  ...
  raw.recipes = raw.recipes.map(function(recipe) {
    if (recipe.category === categoryName) {
      const updated = Object.assign({}, recipe, { category: '' });
      affectedRecipes.push(updated);
      return updated;
    }
    return recipe;
  ...
  state.customRecipeCategories = state.customRecipeCategories.filter(function(category) {
...
async function saveRecipeFromDialog() {
  ...
  const categoryName = document.getElementById('recipeCategoryInput').value.trim();
  const recipeRecord = {
    id: state.editingRecipeId || ('REC' + Date.now()),
    name: name,
  ...
  if (categoryName && !state.customRecipeCategories.includes(categoryName)) {
    state.customRecipeCategories.push(categoryName);
  }
  ...
  applyLoadedState({
    ...
    source: 'local',
    statusText: state.backendUrl ? 'บันทึกเมนูแล้ว กำลังส่งขึ้น Google Sheet' : 'บันทึกเมนูแล้ว',
  });
  ...
  if (state.backendUrl) {
    try {
      ...
      console.error(error);
      updateBackendStatus('บันทึกเมนูบน Google Sheet ไม่สำเร็จ ตอนนี้ข้อมูลอยู่ในเครื่องก่อน', 'error');
    }
  }
}
...
async function saveIngredientFromDialog() {
  ...
  applyLoadedState({
    ...
    source: 'local',
    statusText: state.backendUrl ? 'เพิ่มวัตถุดิบแล้ว กำลังส่งขึ้น Google Sheet' : 'เพิ่มวัตถุดิบแล้ว',
  });
  ...
  if (state.backendUrl) {
    try {
      ...
      console.error(error);
      updateBackendStatus('บันทึกวัตถุดิบบน Google Sheet ไม่สำเร็จ ตอนนี้ข้อมูลอยู่ในเครื่องก่อน', 'error');
    }
  }
}
...
window.openRecipeCategoryDialog = openRecipeCategoryDialog;
window.editRecipeCategoryName = editRecipeCategoryName;
window.deleteRecipeCategoryName = deleteRecipeCategoryName;
